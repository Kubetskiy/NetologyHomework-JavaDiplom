...
public class ImageToSymbols implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        if (maxRatio > 0) {
            ...
            if (imgRatio > maxRatio || 1 / imgRatio > maxRatio) {
        ...
        if (maxWidth > 0 || maxHeight > 0) { // Заданы ограничения на размер картинки
            if (img.getWidth() > maxWidth || img.getHeight() > maxHeight) { // Картинку нужно масштабировать
                double imgRatio = (double) img.getWidth() / (double) img.getHeight();
                ...
                if (maxWidth == 0) { // Задана только максимальная высота
                    newHeight = maxHeight;
                    newWidth = (int) (newHeight * imgRatio);
                }
